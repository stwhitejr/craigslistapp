...
var debug = (0, _debug2.default)('craigslist'),
    ...
    QUERY_KEYS = ['bundleDuplicates', 'category', 'hasImage', 'hasPic', 'maxAsk', 'minAsk', 'offset', 'postal', 'postedToday', 'query', 'searchDistance', 'searchNearby', 'searchTitlesOnly', 'srcType', 'minSqft', 'maxSqft', 'minBedrooms', 'maxBedrooms'],
    ...
    QUERY_PARAM_MIN_BEDROOM = '&min_bedrooms=',
    QUERY_PARAM_MAX_BEDROOM = '&max_bedrooms=',
...
function _getPostings(options, markup) {
	...
	$('div.content').find('.result-row').each(function (i, element) {
		...
		// fix for #6 and #24
		detailsUrl = (0, _urlParse2.default)($(element).find('.result-title').attr('href'));
		...
		posting = {
			...
			housing: ($(element).find('.result-meta .housing').text() || '').trim(),
			area: ($(element).find('.result-meta .result-hood').text() || '').trim(),
...
function _getRequestOptions(options, query) {
	...
	if (!_core2.default.Validation.isEmpty(options.postal)) {
	...
	if (!_core2.default.Validation.isEmpty(options.searchDistance)) {
	...
	// add minBedrooms (if specified)
	if (!_core2.default.Validation.isEmpty(options.minBedrooms)) {
		requestOptions.path = [requestOptions.path, QUERY_PARAM_MIN_BEDROOM, options.minBedrooms].join('');
	}

	// add maxBedrooms (if specified)
	if (!_core2.default.Validation.isEmpty(options.maxBedrooms)) {
		requestOptions.path = [requestOptions.path, QUERY_PARAM_MAX_BEDROOM, options.maxBedrooms].join('');
	}

	// add minSqft (if specified)
	if (!_core2.default.Validation.isEmpty(options.minSqft)) {
		requestOptions.path = [requestOptions.path, QUERY_PARAM_MIN_SQFT, options.minSqft].join('');
	}

	// add maxSqft (if specified)
	if (!_core2.default.Validation.isEmpty(options.maxSqft)) {
		...
		value: function details(posting, callback) {
			...
			exec = new Promise(function (resolve, reject) {
				return getDetails.then(function (details) {
					...
					details.replyUrl = (0, _urlParse2.default)(details.replyUrl);
